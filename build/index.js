...
function isBracketTagLine(trimmedLine) {
    return /^\[[^\]]+\]$/u.test(trimmedLine);
}
function parseScheme(scheme, lineCount) {
    const s = (scheme ?? "").trim();
    if (!s)
        return null;
    ...
    if (letters.length === 0)
        return null;
    if (letters.length >= lineCount)
        return letters.slice(0, lineCount);
    const out = [];
    for (let i = 0; i < lineCount; i++)
        out.push(letters[i % letters.length] ?? "A");
    return out;
}
function pickRhymeKey(analyzed, strictness) {
    if (!analyzed.rhymeKeys)
        return null;
    if (strictness === "final")
        return analyzed.rhymeKeys.final;
    if (strictness === "finalTone")
        return analyzed.rhymeKeys.finalTone;
    if (strictness === "wide")
        return analyzed.rhymeKeys.final;
...
function splitTrailingPunctuation(line) {
    const m = line.match(/[ \t\r\n,.;:!?'"“”‘’（）()「」【】\[\]，。；：！？、…]+$/u);
    if (!m)
        return { stripped: line, trailing: "" };
    const trailing = m[0] ?? "";
...
server.tool("cantonese_rhyme_analyze_end", "Analyze the end of a Cantonese line and return rhyme keys.", {
    text: z.string()
}, async ({ text }) => {
...
server.tool("cantonese_rhyme_candidates", "Return Cantonese rhyme candidates by a rhyme key and strictness.", {
    rhymeKey: z.string(),
    ...
    if (context)
        engine.learnFromText(context);
    for (const k of contextKeywords ?? [])
        engine.learnFromText(k);
...
server.tool("cantonese_rhyme_diagnose_lyrics", "Analyze a whole Cantonese lyrics block for rhyme consistency and suggestions.", {
    lyrics: z.string(),
    strictness: z.enum(["final", "finalTone", "group", "wide"]).default("final"),
    ...
    context: z.string().optional(),
    contextKeywords: z.array(z.string()).optional()
}, async ({ lyrics, strictness, scheme, toneConsistency, ignoreBracketTags, rewrite, rewriteSuggestionLimit, suggestionLimit, preferredCharLength, context, contextKeywords }) => {
    ...
    const labelToTargetKey = new Map();
    const labelToTargetTone = new Map();
    const labelToTargetToneClass = new Map();
    for (let i = 0; i < perLine.length; i++) {
        const label = labels ? labels[i] ?? "A" : null;
        const key = perLine[i]?.rhymeKey ?? null;
        if (label && key && !labelToTargetKey.has(label))
            labelToTargetKey.set(label, key);
        const tone = perLine[i]?.analyzed.tone ?? null;
        if (label && tone && !labelToTargetTone.has(label))
            labelToTargetTone.set(label, tone);
        const tc = perLine[i]?.toneClass ?? null;
        if (label && tc && !labelToTargetToneClass.has(label))
            labelToTargetToneClass.set(label, tc);
    }
    ...
    const results = perLine.map((row, i) => {
        ...
        const suggestions = targetKey && !ok
            ...
            const existing = map.get(r.label) ??
                {
                    ...
                    okCount: 0,
                    toneSet: new Set(),
                    toneClassSet: new Set()
                };
            existing.indices.push(r.index);
            if (r.ok)
                existing.okCount += 1;
            if (r.analyzed.tone)
                existing.toneSet.add(r.analyzed.tone);
            if (r.toneClass)
                existing.toneClassSet.add(r.toneClass);
    ...
    const rewriteOutput = (() => {
        ...
        const lineRewrites = results.map((r) => {
            ...
            if (!label || !targetKey) {
                return { index: r.index, original: r.text, rewritten: r.text, applied: false, reason: "missing_scheme" };
            }
            if (r.analyzed.rhymeKeys === null) {
                return { index: r.index, original: r.text, rewritten: r.text, applied: false, reason: "no_pronunciation" };
            }
            if (r.ok) {
                return { index: r.index, original: r.text, rewritten: r.text, applied: false, reason: "already_ok" };
            }
            const from = r.analyzed.matchedText;
            if (!from) {
                return { index: r.index, original: r.text, rewritten: r.text, applied: false, reason: "no_match" };
            }
            ...
            if (!best) {
                return { index: r.index, original: r.text, rewritten: r.text, applied: false, reason: "no_candidate" };
            }
            const applied = applyEndReplacement({ originalLine: r.text, analyzed: r.analyzed, replacementText: best });
            if (!applied) {
                return { index: r.index, original: r.text, rewritten: r.text, applied: false, reason: "not_at_end" };
            }
            return {
                index: r.index,
                original: r.text,
        ...
        for (const r of lineRewrites) {
            ...
            if (idx < rewrittenLines.length)
                rewrittenLines[idx] = r.rewritten;
        }
